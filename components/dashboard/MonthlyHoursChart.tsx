...
export default function MonthlyHoursChart({ activities }: MonthlyHoursChartProps) {
  const monthlyData = useMemo(() => {
    ...
    activities.forEach((act) => {
      ...
      const hours = Number(act.volunteering_hours || act.internship_hours || 0);
      groups[monthKey] = (groups[monthKey] || 0) + hours;
    });
  ...
  const chartParams = useMemo(() => {
    ...
    const points = monthlyData.map((d, index) => {
      const x = paddingLeft + (monthlyData.length > 1 
        ...
        : chartWidth / 2);
      const y = height - paddingBottom - (d.hours / maxHours) * chartHeight;
      return { x, y, label: d.displayMonth, val: d.hours };
    });
    ...
    return { points, pathD, areaD, maxHours, paddingLeft, paddingBottom, width, height };
  }, [monthlyData]);
    ...
    return (
      <article className="border border-border bg-white p-5 rounded-2xl font-display flex flex-col justify-between min-h-[260px] h-full">
        <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
          ...
          <h3 className="text-sm font-bold text-ink uppercase tracking-wider">Hours Per Month</h3>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-paper/30 border border-dashed border-border rounded-xl">
          <p className="text-xs font-bold text-mist">No monthly contribution data available.</p>
          <p className="text-[10px] text-mist mt-1">Submit activity logs to populate chart trends.</p>
        </div>
      </article>
    );
  }
  ...
  return (
    <article className="border border-border bg-white p-5 rounded-2xl font-display flex flex-col justify-between min-h-[260px] h-full">
      <div className="flex items-center gap-2 border-b border-border pb-3 mb-2">
        <BarChart3 size={18} className="text-brand" />
        <h3 className="text-sm font-bold text-ink uppercase tracking-wider">Hours Contributed Per Month</h3>
      </div>

      <div className="w-full relative mt-2">
        <svg viewBox={`0 0 ${params.width} ${params.height}`} className="w-full h-auto overflow-visible" aria-label="Monthly hours line chart tracking your logged volunteer and intern hours.">
          <title>Monthly Hours Line Chart</title>
          <desc>This chart details hours plotted against the calendar months of logged contributions.</desc>
          ...
          <text x={params.paddingLeft - 8} y={24} textAnchor="end" className="text-[9px] fill-mist font-bold font-mono">{params.maxHours}h</text>
          <text x={params.paddingLeft - 8} y={94} textAnchor="end" className="text-[9px] fill-mist font-bold font-mono">{Math.round(params.maxHours / 2)}h</text>
          <text x={params.paddingLeft - 8} y={params.height - params.paddingBottom + 3} textAnchor="end" className="text-[9px] fill-mist font-bold font-mono">0h</text>
